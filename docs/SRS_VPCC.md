...
flowchart TD
    ...
    SY_Calc --> SY_Init[6a. Khách duyệt -> Khởi tạo hồ sơ & Duyệt Checklist/UCHI]
    SY_Init --> SY_Template[7a. Tự động điền biểu mẫu Lời chứng]
    SY_Template --> CCV_Sign_SY[8a. Trình CCV ký offline]
    CCV_Sign_SY --> CollectFee_SY[9a. Thu phí tại quầy & Xác nhận trên PM]
    CollectFee_SY --> SY_Scan[10a. Tải lên bản đã ký & Lưu trữ hồ sơ]
    ...
    DT_Config --> DT_CALCULATE[6b. Tính toán chi phí & Báo giá]
    ...
    CTV_Sign_DT --> CollectFee_DT[13b. Thu phí tại quầy & Xác nhận trên PM]
    CollectFee_DT --> DT_Scan[14b. Scan bản dịch dấu đỏ & Tải lên]
    ...
    CT_Print --> CCV_Sign_CT[7c. CCV ký Lời chứng & Đóng dấu offline]
    CCV_Sign_CT --> CollectFee_CT[8c. Thu phí tại quầy & Xác nhận trên PM]
    CollectFee_CT --> CT_Scan[9c. Hoàn tất form & Scan số hóa]

    %% Common End Phase
    SY_Scan --> AccReconcile[Đối soát dòng tiền thực nhận]
...
4. **Tính phí, Báo phí & Thiết lập đặt cọc:**
   ...
   - **Cấu hình Đặt cọc (nếu có):** Trường hợp hồ sơ có giá trị lớn hoặc yêu cầu cọc, Thư ký mở phân hệ Đặt cọc để chọn:
     1. Nhập trực tiếp Số tiền cọc khách hàng đóng.
     2. Hoặc nhập/áp dụng theo Mã đặt cọc tự quy định (mã ưu đãi, gói cọc hoặc mã giao dịch ngân quỹ).
     - Thư ký tích chọn/nhấn **"Xác nhận đã nhận cọc"** để hệ thống ghi nhận cọc.
   - Thư ký báo phí và cọc với khách hàng để khách hàng chốt.
5. **Khởi tạo hồ sơ & Thẩm định (Khi khách duyệt):**
   - Khi khách hàng đồng ý báo phí, Thư ký nhấn nút "Khởi tạo hồ sơ" (Lưu hồ sơ). Hệ thống tạo mã hồ sơ duy nhất (dạng `SY-YYYYMMDD-XXXXX`) ở trạng thái "Đang xử lý".
...
7. **Trình CCV ký offline (Ký tên & Đóng dấu):**
...
8. **Thu phí tại quầy:**
   - Thư ký nhận tiền mặt hoặc cho khách quét mã QR động chuyển khoản hiển thị trên màn hình.
   - Chọn phương thức thanh toán tương ứng trên phần mềm và nhấn "Xác nhận nhận đủ tiền". Trạng thái hồ sơ chuyển sang "Chờ tải file scan".
9. **Tải lên bản đã ký & Lưu trữ hồ sơ:**
   - Thư ký đặt tài liệu đã có chữ ký, con dấu đỏ của CCV vào máy scan tại quầy, quét và tải file scan (định dạng PDF/JPG, tối đa 25MB) lên hồ sơ phần mềm để lưu trữ điện tử.
   - Thư ký nhấn nút "Hoàn tất xử lý hồ sơ". Trạng thái hồ sơ chuyển sang "Chờ đối soát".
10. **Đối soát & Phát hành Hóa đơn:**
...
1. **Tiếp nhận, Phân loại, CRM & Khởi tạo quản trị:**
...
4. **Yêu cầu Tạm ứng (Đặt cọc) & Kích hoạt luồng dịch:**
    - Đối với các hồ sơ có giá trị lớn (doanh nghiệp B2B) hoặc ngôn ngữ hiếm, hệ thống tự động bật cờ `Yêu cầu tạm ứng` và khóa tính năng phân công cho đến khi Thư ký xác nhận đã thu trước tối thiểu 50% tiền cọc.
    - **Cơ chế đặt cọc:** Thư ký thực hiện nhập tiền cọc theo 2 hình thức:
      1. Nhập trực tiếp Số tiền cọc khách hàng đóng.
      2. Hoặc nhập/áp dụng theo Mã đặt cọc tự quy định của văn phòng.
      - Thư ký tích chọn/nhấn **"Xác nhận đã nhận cọc"** để ghi nhận tiền cọc lên hệ thống, mở khóa tính năng phân công xử lý.
...
8. **Thu phí tại quầy:**
    - Khách hàng thanh toán tại quầy (hoặc thanh toán nốt 50% tiền còn lại nếu có cọc).
    - Thư ký nhấn **"Xác nhận nhận đủ tiền"** trên hệ thống. Trạng thái hồ sơ chuyển sang "Chờ tải file scan".
9. **Scan số hóa & Lưu trữ hồ sơ:**
    - Thư ký đặt tập tài liệu đã đóng dấu đỏ hoàn chỉnh vào máy scan tại quầy, quét và tải file scan lên hồ sơ để lưu trữ điện tử.
    - Thư ký nhấn nút "Hoàn tất xử lý hồ sơ". Trạng thái hồ sơ chuyển sang "Chờ đối soát".
10. **Đối soát & Phát hành Hóa đơn:**
...
4. **Báo phí & Thiết lập đặt cọc:**
   ...
   - **Cơ chế Đặt cọc (nếu có):** Nếu khách hàng cần đặt cọc trước thù lao, Thư ký mở mục Đặt cọc để chọn:
     1. Nhập trực tiếp Số tiền cọc khách hàng đóng.
     2. Hoặc nhập/áp dụng theo Mã đặt cọc tự quy định.
     - Sau khi nhận tiền, Thư ký nhấn **"Xác nhận đã nhận cọc"** để ghi nhận giao dịch cọc lên phần mềm.
...
7. **Thu phí tại quầy:**
   - Thư ký tiến hành thu tiền mặt/chuyển khoản từ khách hàng theo tổng thực thu đã chốt, nhấn **"Xác nhận nhận đủ tiền"** trên hệ thống. Trạng thái hồ sơ chuyển sang "Chờ tải file scan".
8. **Hoàn tất form & Scan số hóa tài liệu:**
   - Thư ký xin thông tin khách hàng để nhập hoàn thiện các trường còn thiếu trên form hồ sơ (nếu trước đó đã bỏ qua ở bước 3).
   - Thư ký scan bản văn bản đã ký đóng dấu đầy đủ có chữ ký và dấu đỏ của CCV, tải file scan lên hệ thống và bấm **"Hoàn tất xử lý"**. Trạng thái hồ sơ chuyển sang "Chờ đối soát".
9. **Đối soát & Phát hành Hóa đơn:**
...
![Quản lý nhân sự & Phân quyền](images/user_management_mockup.png)
...
| **Luồng sự kiện chính (Thành công)** | **STT \| Thực hiện bởi \| Hành động** <br>1. Thư ký \| Nhập Số điện thoại/CCCD (Cá nhân) hoặc Mã số thuế (Doanh nghiệp) vào ô tìm kiếm. <br>2. Hệ thống \| Tìm kiếm trong CRM: Nếu có dữ liệu cũ, tự động điền (Auto-fill) thông tin định danh. Nếu chưa có, mở form thêm mới khách hàng. <br>3. Thư ký \| Chọn loại nghiệp vụ (Sao y bản chính, Chứng thực chữ ký, Dịch thuật công chứng). <br>4. Thư ký \| Chọn loại giấy tờ (CCCD, Sổ đỏ, Bằng cấp, Giấy phép kinh doanh,...) từ danh sách dropdown. <br>5. Thư ký \| Nhập các thông số kỹ thuật (Số trang gốc, Số bản cần làm), chọn **Nguồn khách hàng** để tính KPI, chọn **Người phụ trách** xử lý, và chọn **Công chứng viên (CCV)** chịu trách nhiệm ký duyệt. <br>6. Hệ thống \| Tự động tính phí gốc theo công thức của Nhà nước. <br>7. Thư ký \| Nhập số tiền thực tế thống nhất thu vào trường "Tổng thực thu" và báo phí cho khách. <br>8. Thư ký \| Thiết lập đặt cọc nếu có (nhập số tiền cọc hoặc chọn theo mã cọc tự quy định, tích chọn đã nhận cọc nếu khách đóng trước), tích chọn checkbox `Khách hàng đồng ý báo phí` và nhấn "Lưu hồ sơ". <br>9. Hệ thống \| Khởi tạo hồ sơ, tự động sinh Mã hồ sơ duy nhất, đặt trạng thái hồ sơ thành "Đang xử lý". |
...
| 12 | Số tiền cọc | Số tiền cọc đóng trước (nếu có) | Không | Số tiền $\ge$ 0 | 500000 |
| 13 | Mã đặt cọc | Mã đặt cọc tự quy định của văn phòng | Không | Đúng định dạng mã cấu hình | COC-500K-01 |
| 14 | Đã nhận cọc | Cờ xác nhận đã thu cọc | Không | Nhận giá trị Boolean | True |

###### Giao diện mẫu (Prototype) - Khởi tạo hồ sơ:
![Giao diện Khởi tạo hồ sơ](images/create_dossier_mockup.png)
...
![Thẩm định & Checklist chặn](images/appraisal_checklist_mockup.png)
...
![Quét & Lưu trữ hồ sơ](images/scan_archive_mockup.png)
...
![Đối soát & Hóa đơn điện tử](images/invoice_reconciliation_mockup.png)
...
![Chăm sóc khách hàng qua Zalo OA](images/zalo_customer_care_mockup.png)
...
![Sổ công chứng điện tử](images/notary_ledger_mockup.png)
