...
- **[Chương 2. Các yêu cầu chức năng (Functional Requirements)](#chương-2-các-yêu-cầu-chức-năng-functional-requirements)**
  ...
  - [2.5 Quy trình sử dụng phần mềm (Software User Workflow)](#2.5-quy-trinh-su-dung-phan-mem)
    - [2.5.A Quy trình Nghiệp vụ Sao y bản chính](#2.5.a-quy-trinh-nghiep-vu-sao-y-ban-chinh)
    - [2.5.B Quy trình Nghiệp vụ Dịch thuật công chứng](#2.5.b-quy-trinh-nghiep-vu-dich-thuat-cong-chung)
    - [2.5.C Quy trình Nghiệp vụ Chứng thực chữ ký](#2.5.c-quy-trinh-nghiep-vu-chung-thuc-chu-ky)
...
Dự án được phát triển nhằm mục tiêu xây dựng một hệ thống quản trị tổng thể (CRM) chuyên biệt cho văn phòng công chứng. Hệ thống số hóa toàn bộ quy trình tiếp nhận hồ sơ tại quầy, (thẩm định pháp lý) và kiểm soát lỗi qua checklist chặn, tra cứu dữ liệu ngăn chặn giao dịch (UCHI), quản lý dòng tiền hạch toán và tự động xuất hóa đơn VAT, cũng như chăm sóc khách hàng tự động qua Zalo OA. Dự án giúp tối ưu hóa hiệu suất làm việc (Thư ký, Công chứng viên, Kế toán, Admin), giảm thiểu 95% sai sót nghiệp vụ và nâng cao trải nghiệm dịch vụ khách hàng.
...
1. **Thư ký nghiệp vụ (Secretary):**
   ...
   - **Nhiệm vụ trên hệ thống:**
     ...
     - Thực hiện kiểm tra các đầu mục của checklist động tương ứng với loại giấy tờ, dịch vụ.
     ...
     - Thu phí dịch vụ của khách hàng tại quầy (Tiền mặt hoặc chuyển khoản) và bấm "Xác nhận đã thu phí" trên hệ thống.
...
2. **Công chứng viên (Notary Officer):**
   - **Mô tả:** Người có thẩm quyền tư pháp chịu trách nhiệm xem xét, phê duyệt và ký lời chứng.
   - **Nhiệm vụ trên hệ thống:**
     - Công chứng viên có toàn bộ quyền hạn của Thư ký nghiệp vụ trên hệ thống (có thể tiếp nhận, xử lý hồ sơ và thu phí dịch vụ).
     - Thực hiện ký đóng dấu bản cứng offline sau khi đã đối chiếu và duyệt trên hệ thống

3. **Kế toán (Accountant):**
   - **Mô tả:** Nhân sự phụ trách đối soát tài chính và quản lý hóa đơn tại văn phòng công chứng.
   - **Nhiệm vụ trên hệ thống:**
     - Theo dõi danh sách giao dịch ở trạng thái "đã thanh toán".
     - Thực hiện đối soát tài chính: đối chiếu dòng tiền thực nhận (tiền mặt thực tế hoặc tài khoản ngân hàng của văn phòng) với thông tin thu phí do Thư ký/CCV đã xác nhận.
...
- **Chăm sóc khách hàng:** Tự động gửi tin nhắn chăm sóc, thông báo nhận kết quả và link hóa đơn VAT qua Zalo OA.
...
- **Checklist động & Kiểm soát lỗi:** checklist các danh sách kiểm tra bắt buộc tùy thuộc vào loại giấy tờ để ngăn ngừa lỗi kỹ thuật của Thư ký hoặc CCV (Kiểm tra thông tin trên hệ thống UCHI của Sở Tư pháp để tra cứu trạng thái phong tỏa của thửa đất/tài sản rủi ro cao.)

- **Soạn thảo:** Tự động điền dữ liệu hồ sơ vào biểu mẫu Lời chứng có sẵn, quản lý các phiên bản tài liệu (Nháp, Đang xử lý, Chờ ký, Đã ký).
...
- **Thu phí dịch vụ tại quầy:** Thư ký/CCV tiếp nhận thanh toán từ khách hàng (Tiền mặt, Chuyển khoản) và xác nhận thu phí trên hệ thống.
- **Đối soát dòng tiền:** Kế toán đối chiếu dòng tiền thực nhận (tiền mặt, tài khoản ngân hàng) với giao dịch thu phí trên hệ thống.
- **Xuất hóa đơn điện tử:** Hệ thống tự động gọi API VNPT/Vĩnh Hy phát hành hóa đơn VAT sau khi Kế toán xác nhận đối soát.
- **Chăm sóc khách hàng:** Tự động gửi tin nhắn cảm ơn và link hóa đơn qua Zalo OA.
...
flowchart TD
    ...
    %% Branch 1: Sao y
    ChooseBranch -- Sao y bản chính --> SY_DocType[3a. Chọn loại giấy tờ sao y]
    SY_DocType --> SY_Config[4a. Cấu hình số trang & số bản]
    SY_Config --> SY_Calc[5a. Tính phí & Báo phí khách hàng]
    SY_Calc --> SY_Init[6a. Khách duyệt -> Khởi tạo hồ sơ: Phân rã N bản & Cấp N số Nháp sớm UC005]
    SY_Init --> SY_Checklist[7a. Thẩm định hồ sơ & Duyệt Checklist/UCHI UC006]
    SY_Checklist --> SY_Template[8a. Sinh & In Lời chứng đã có số Nháp UC007]
    SY_Template --> CCV_Sign_SY_Offline[9a. Trình CCV ký đóng dấu vật lý offline]
    CCV_Sign_SY_Offline --> CCV_Approve_Confirm[10a. CCV Đối soát số Nháp & Phê duyệt ký số CA UC012]
    CCV_Approve_Confirm --> CollectFee_SY[11a. Thu phí tại quầy: Tự động chuyển số sang Hoàn thành UC009]
    CollectFee_SY --> SY_Scan[12a. Tải lên bản đã scan & Lưu trữ hồ sơ UC008]

    %% Branch 2: Dịch thuật
    ChooseBranch -- Dịch thuật công chứng --> DT_Input[3b. Khởi tạo hồ sơ & Cấp số bản dịch Nháp sớm UC005]
    DT_Input --> DT_Config[4b. Chọn ngôn ngữ nguồn, đích & chọn loại tài liệu]
    DT_Config --> DT_CHECKLIST[5b. Kiểm tra tài liệu & Duyệt Checklist chặn UC006]
    DT_CHECKLIST --> DT_CALCULATE[6b. Tính toán chi phí & Báo giá]
    DT_CALCULATE --> DT_DEPOSIT[7b. Yêu cầu khách chuyển tiền tạm ứng / đặt cọc trước]
    ...
    DT_Translate --> DT_Check[11b. KH, thư ký duyệt chất lượng]
    DT_Check --> DT_Template[12b. Sinh & In Lời chứng dịch thuật đã có số Nháp UC007]
    DT_Template --> CTV_Sign_DT_Offline[13b. CTV ký mẫu & CCV ký đóng dấu vật lý offline]
    CTV_Sign_DT_Offline --> CCV_Approve_Confirm_DT[14b. CCV Đối soát số Nháp & Phê duyệt ký số CA UC012]
    CCV_Approve_Confirm_DT --> CollectFee_DT[15b. Thu phí tại quầy: Tự động chuyển số sang Hoàn thành UC009]
    CollectFee_DT --> DT_Scan[16b. Scan bản dịch dấu đỏ & Tải lên UC008]

    %% Branch 3: Chứng thực chữ ký
    ChooseBranch -- Chứng thực chữ ký --> CT_Create[1c. Tạo hồ sơ & Cấp số chứng thực Nháp sớm UC005]
    ...
    CT_Fee --> CT_Checklist[5c. Checklist nghiệp vụ động & Duyệt checklist chặn UC006]
    CT_Checklist --> CT_Print[6c. Sinh & In Lời chứng đã có số Nháp UC007]
    CT_Print --> CCV_Sign_CT_Offline[7c. KH ký & CCV ký đóng dấu vật lý offline]
    CCV_Sign_CT_Offline --> CCV_Approve_Confirm_CT[8c. CCV Đối soát số Nháp & Phê duyệt ký số CA UC012]
    CCV_Approve_Confirm_CT --> CollectFee_CT[9c. Thu phí tại quầy: Tự động chuyển số sang Hoàn thành UC009]
    CollectFee_CT --> CT_Scan[10c. Hoàn tất form & Scan số hóa UC008]

    %% Common End Phase
    SY_Scan --> AccReconcile[Đối soát dòng tiền thực nhận UC010]
    DT_Scan --> AccReconcile
    CT_Scan --> AccReconcile

    AccReconcile --> SysInvoice[Tự động bóc tách VAT & Gọi API Xuất Hóa đơn]
    SysInvoice --> ZaloSend[Gửi link Hóa đơn VAT & Cảm ơn qua Zalo OA UC011]
    ZaloSend --> EndNode([Hoàn tất hồ sơ])
```

<a id="2.5.a-quy-trinh-nghiep-vu-sao-y-ban-chinh"></a>
#### 2.5.A Quy trình Nghiệp vụ Sao y bản chính (Certified Copying Workflow)

Quy trình sử dụng phần mềm đối với nghiệp vụ Sao y bản chính được thực hiện tuần tự như sau:

1. **Tiếp nhận, Định danh Khách hàng (CRM) & Khởi tạo quản trị:**
   - Thư ký nghiệp vụ xin số điện thoại của khách hàng.
   - Thư ký nhập thông tin vào ô số điện thoại.
   ...
   - **Nhập thông tin quản trị:** Thư ký chọn **Nguồn khách hàng** (Nguồn đối tác liên kết, Nguồn cá nhân nhân viên khai thác...) để tính KPI/hoa hồng; chọn **Người phụ trách** xử lý hồ sơ (Thư ký nghiệp vụ phụ trách); và chọn **Công chứng viên (CCV)** chịu trách nhiệm ký duyệt hồ sơ.
2. **Chọn loại giấy tờ sao y:**
   - Thư ký chọn loại giấy tờ cần sao y từ danh sách cấu hình sẵn (ví dụ: CCCD, Sổ đỏ, Bằng đại học, Học bạ...).
3. **Cấu hình số trang & số bản:**
   - Thư ký nhập số trang của tài liệu gốc và số bản sao y khách hàng yêu cầu.
4. **Tính phí, Báo phí & Thiết lập đặt cọc:**
   - **Hệ thống tự động tính toán:** Tính phí gốc theo quy định nhà nước ($Phí\ gốc = Số\ trang \times Số\ bản \times 3.000đ$).
   - **Cấu hình Tạm ứng / Đặt cọc (nếu có):** Trường hợp hồ sơ có giá trị lớn hoặc yêu cầu cọc, Thư ký mở phân hệ Đặt cọc để chọn:
     1. Nhập trực tiếp Số tiền tạm ứng / đặt cọc khách hàng đóng.
     2. Hoặc nhập/áp dụng theo Mã tạm ứng / đặt cọc tự quy định (mã ưu đãi, gói tạm ứng/cọc hoặc mã giao dịch ngân quỹ).
     - Thư ký tích chọn/nhấn **"Xác nhận đã nhận tạm ứng / đặt cọc"** để hệ thống ghi nhận tạm ứng / đặt cọc.
   - Thư ký báo phí và cọc với khách hàng để khách hàng chốt.
5. **Khởi tạo hồ sơ, Cấp số Nháp sớm (UC005) & Phân rã dịch vụ con (Khi khách duyệt):**
   - Khi khách hàng đồng ý báo phí, Thư ký nhấn nút "Khởi tạo hồ sơ".
   - **Tự động phân rã:** Hệ thống tự động phân tách số bản sao y $N$ thành $N$ dòng dịch vụ con độc lập (ví dụ: Bản 1/N, Bản 2/N...).
   - **Cấp số Nháp sớm:** Hệ thống tự động truy cập Sổ sao y tương ứng trên Sổ điện tử để **xin cấp $N$ số chứng thực liên tiếp sớm ở trạng thái Nháp (Draft / Tạm giữ)** để giữ chỗ, tránh bị trùng hoặc bị cấp cho hồ sơ khác. Các số Nháp này hiển thị trực tiếp trên bảng dịch vụ con (bảng này hiển thị rõ cột Số trang gốc của từng tài liệu).
   - Hệ thống tạo mã hồ sơ duy nhất (dạng `SY-YYYYMMDD-XXXXX`) ở trạng thái "Đang xử lý".
6. **Thẩm định hồ sơ & Duyệt Checklist chặn (UC006):**
   ...
   - *Đối với các giấy tờ sở hữu tài sản (như Sổ đỏ/Sổ hồng):* Thư ký bắt buộc phải click vào nút "UCHI Search". Hệ thống gọi API thời gian thực đến CSDL ngăn chặn của Sở Tư pháp.
     - **Nếu có ngăn chặn:** Hệ thống khóa nút, đổi trạng thái hồ sơ thành "Bị ngăn chặn" và gửi cảnh báo đỏ. Thư ký thực hiện trả hồ sơ cho khách.
     - **Nếu bình thường:** Hệ thống mở khóa bước tiếp theo.
7. **Tự động điền & Tạo Lời chứng (UC007):**
   - Thư ký nhấn nút "In Lời chứng" tương ứng với từng dịch vụ con. Hệ thống tự động lấy thông tin từ hồ sơ và **Số chứng thực Nháp đã cấp ở bước 5** điền vào biểu mẫu lời chứng tương ứng (Ví dụ: Số: 1234, Quyển số: 01-SY).
   - Thư ký in Lời chứng ra kẹp vào tài liệu giấy. Hồ sơ chuyển trạng thái sang "Chờ ký (CCV)".
8. **Trình CCV ký offline (Ký tên & Đóng dấu):**
   - Thư ký kẹp bản sao y và Lời chứng đã in (đã có in sẵn số chứng thực tương ứng) trình Công chứng viên (CCV).
   - CCV thực hiện đối chiếu vật lý bản chính và bản sao, ký tên và đóng dấu đỏ vật lý lên tài liệu.
9. **CCV Đối soát số Nháp & Phê duyệt ký số CA (UC012):**
   - CCV đăng nhập hệ thống, mở hồ sơ thuộc danh sách "Chờ phê duyệt".
   - CCV đối soát chéo danh sách các số chứng thực đã cấp sớm (đang ở trạng thái **Nháp - Draft**) hiển thị trên phần mềm với bản cứng đã ký đóng dấu vật lý.
   - CCV chọn chữ ký số CA (USB Token hoặc Smart-CA) và nhấn nút **"Phê duyệt & Ký số CA"**.
   - Hệ thống ghi nhận chữ ký số của CCV lên Lời chứng điện tử, ghi log phê duyệt và chuyển trạng thái hồ sơ sang **"Chờ thu phí"** (số chứng thực vẫn giữ trạng thái Nháp).
   - *Trường hợp khách hàng/CCV hủy hồ sơ:* Người dùng nhấn "Hủy hồ sơ" trên giao diện chi tiết, nhập lý do hủy. Hệ thống cập nhật trạng thái hồ sơ thành "Đã hủy (Có cấp số)" và chuyển trạng thái các số chứng thực tương ứng từ Nháp sang **Đã hủy (Cancelled)** (giữ nguyên trong sổ để lưu vết, cấm thu hồi/giải phóng).
10. **Thu phí tại quầy (UC009):**
    - Thư ký/Thu ngân nhận tiền mặt hoặc cho khách quét mã QR động chuyển khoản hiển thị trên màn hình.
    - Thư ký có thể điều chỉnh số trang/bản để recalculate thù lao và thuế VAT nhằm khớp với tiền thực thu (tự động làm tròn tiền thực thu đến hàng nghìn đồng).
    - Nhấn "Xác nhận nhận đủ tiền". **Hệ thống tự động chuyển đổi trạng thái của toàn bộ các số công chứng đã cấp sớm tương ứng từ Nháp (Draft) sang Hoàn thành (Completed) trên Sổ điện tử**. Trạng thái hồ sơ chuyển sang **"Chờ tải file scan"**.
    - *Trường hợp hủy hồ sơ tại quầy:* Chọn hủy hồ sơ, nhập lý do hủy. Hệ thống cập nhật số chứng thực tương ứng thành **Đã hủy (Cancelled)**. Nếu hồ sơ đã có cọc, xử lý tịch thu tạm ứng / đặt cọc hoặc tạo phiếu chi hoàn tạm ứng/cọc.
11. **Tải lên bản đã ký & Lưu trữ hồ sơ (UC008):**
    - Thư ký đặt tài liệu đã có chữ ký, con dấu đỏ của CCV vào máy scan tại quầy, quét và tải file scan (định dạng PDF/JPG, tối đa 25MB) lên hồ sơ phần mềm để lưu trữ điện tử. Hệ thống hỗ trợ quét mã QR Code độc bản trên Lời chứng để tự động nhận dạng.
    - Thư ký nhấn nút "Hoàn tất xử lý hồ sơ". Trạng thái hồ sơ chuyển sang "Chờ đối soát".
12. **Đối soát & Phát hành Hóa đơn (UC010):**
...
<a id="2.5.b-quy-trinh-nghiep-vu-dich-thuat-cong-chung"></a>
#### 2.5.B Quy trình Nghiệp vụ Dịch thuật công chứng (Certified Translation Workflow)
...
1. **Tiếp nhận, Phân loại, CRM, Cấp số Nháp sớm (UC005) & Khởi tạo quản trị:**
    - Thư ký bấm nút khởi tạo hồ sơ.
    - **Luồng tại quầy:** Thư ký nghiệp vụ tiếp nhận bản chính tài liệu, nhập Số điện thoại/CCCD (Cá nhân) hoặc Mã số thuế (Doanh nghiệp) vào CRM để tự động điền (Auto-fill) thông tin đương sự.
    - **Cấp số Nháp sớm:** Hệ thống tự động truy cập Sổ bản dịch tương ứng trên Sổ điện tử để **xin cấp 1 số chứng thực bản dịch sớm ở trạng thái Nháp (Draft / Tạm giữ)** để giữ chỗ, tránh trùng số. Số Nháp này hiển thị trực tiếp trên giao diện hồ sơ.
    ...
    - **Nhập thông tin quản trị:** Thư ký chọn **Nguồn khách hàng** (Nguồn đối tác liên kết, Nguồn cá nhân nhân viên khai thác...) để tính KPI/hoa hồng; chọn **Người phụ trách** xử lý hồ sơ; và chọn **Công chứng viên (CCV)** chịu trách nhiệm ký duyệt hồ sơ.
...
2. **Kiểm soát rủi ro pháp lý & Chặn nghiệp vụ (Checklist động - UC006):**
...
3. **Áp dụng Mô hình Giá lai linh hoạt (Hybrid Pricing Model):**
    ...
    - **Cơ chế ghi đè giá (Price Override):** Thư ký được quyền nhập trực tiếp số tiền thực tế thỏa thuận với khách hàng vào trường `Tổng thực thu` (bao gồm phí dịch thuật chuyên ngành, phí dịch vụ hỏa tốc, in ấn).
    ...
    - **Quản lý đa ngôn ngữ (Master - Sub Dossier):** Nếu tài liệu gốc cần dịch ra nhiều ngôn ngữ cùng lúc (ví dụ: Anh, Pháp, Đức), Thư ký bấm **"Thêm nhánh dịch thuật"**. Hệ thống tự động sinh ra các hồ sơ con độc lập (`DT-SUB-ANH`, `DT-SUB-PHAP`...) liên kết với một `Hồ sơ cha (MASTER)` duy nhất để gộp biểu phí và xuất một hóa đơn VAT tổng. Mỗi hồ sơ con sẽ được hệ thống cấp 1 số chứng thực bản dịch Nháp tương ứng trên Sổ điện tử.
4. **Yêu cầu Tạm ứng (Đặt cọc) & Kích hoạt luồng dịch:**
    - Đối với các hồ sơ có giá trị lớn (doanh nghiệp B2B) hoặc ngôn ngữ hiếm, hệ thống tự động bật cờ `Yêu cầu tạm ứng` và khóa tính năng phân công cho đến khi Thư ký xác nhận đã thu trước tối thiểu 50% tiền tạm ứng / đặt cọc.
    - **Cơ chế đặt cọc:** Thư ký thực hiện nhập tiền tạm ứng / đặt cọc (nhập trực tiếp hoặc qua Mã tạm ứng / đặt cọc tự quy định) và nhấn **"Xác nhận đã nhận tạm ứng / đặt cọc"** để ghi nhận tiền tạm ứng / đặt cọc lên hệ thống, mở khóa tính năng phân công xử lý.
...
6. **Phê duyệt bản nháp trực tuyến & Duyệt chất lượng:**
    - **Gửi bản nháp qua Zalo OA:** Thư ký nhận thông báo file dịch hoàn thành, bấm nút **"Gửi bản nháp duyệt"**. Hệ thống tự động xuất file PDF bản dịch nháp có chèn chữ chìm (Watermark) *"BẢN NHÁP KIỂM TRA - CHƯA CÓ GIÁ TRỊ PHÁP LÝ"* gửi qua Zalo OA cho khách hàng kiểm tra thông tin từ xa.
...
7. **Sinh Lời chứng dịch thuật & Ký chứng thực (UC007):**
    - Hệ thống tự động gọi cấu hình mẫu lời chứng dịch thuật (Mẫu `TEMP-DICH-01`), trích xuất thông tin định danh khách hàng, thông tin ngôn ngữ và thông tin đăng ký tư pháp của CTV để sinh Lời chứng tự động chứa **Số chứng thực bản dịch Nháp đã cấp ở bước 1**. Thư ký thực hiện in ấn toàn bộ tài liệu.
    - **Xác thực chữ ký mẫu từ xa:** CTV ký mẫu (hoặc CCV đối chiếu chữ ký mẫu vật lý trên bản dịch do CTV đã ký sẵn và gửi kèm với ảnh chữ ký mẫu độ phân giải cao của CTV hiển thị trên hệ thống).
    - CCV xem xét hồ sơ, thực hiện ký tên vào lời chứng vật lý và đóng dấu đỏ vật lý offline lên tài liệu kết quả.
8. **CCV Đối soát số Nháp & Phê duyệt ký số CA (UC012):**
    - CCV đăng nhập hệ thống, mở hồ sơ chờ duyệt.
    - CCV đối soát chéo các số chứng thực hiển thị trên màn hình (đang ở trạng thái **Nháp - Draft**) với số thứ tự thực tế ghi trên bản cứng giấy dấu đỏ.
    - CCV nhấn nút **"Phê duyệt & Ký số CA"**.
    - Hệ thống ghi nhận chữ ký số của CCV lên Lời chứng điện tử và chuyển trạng thái hồ sơ sang **"Chờ thu phí"** (số chứng thực vẫn giữ trạng thái Nháp).
    - *Trường hợp hủy hồ sơ:* CCV hoặc Thư ký bấm nút "Hủy hồ sơ" trên giao diện chi tiết, nhập lý do hủy. Hệ thống cập nhật trạng thái hồ sơ thành "Đã hủy (Có cấp số)" và chuyển trạng thái các số chứng thực tương ứng từ Nháp sang **Đã hủy (Cancelled)**.
9. **Thu phí tại quầy (UC009):**
    - Khách hàng thanh toán phần tiền còn lại tại quầy. Thư ký nhấn **"Xác nhận nhận đủ tiền"**.
    - **Hệ thống tự động chuyển đổi trạng thái của số chứng thực tương ứng từ Nháp (Draft) sang Hoàn thành (Completed) trên Sổ điện tử**, chuyển trạng thái hồ sơ sang **"Chờ tải file scan"**.
    - *Trường hợp hủy hồ sơ tại quầy:* Chọn hủy hồ sơ, nhập lý do hủy. Hệ thống cập nhật số chứng thực tương ứng thành **Đã hủy (Cancelled)**.
10. **Scan số hóa & Lưu trữ hồ sơ (UC008):**
    - Thư ký scan tập tài liệu đã đóng dấu đỏ hoàn chỉnh, tải file scan lên hồ sơ để hệ thống tự động nhận diện và liên kết qua mã QR Code.
    - Thư ký nhấn nút "Hoàn tất xử lý hồ sơ". Trạng thái hồ sơ chuyển sang "Chờ đối soát".
11. **Đối soát & Phát hành Hóa đơn (UC010):**
    - Kế toán kiểm tra dòng tiền thực nhận và bấm **"Xác nhận đối soát"**.
    - **Hệ thống tự động phát hành:** Ghi sổ cái kế toán bất biến, gọi API xuất hóa đơn điện tử VAT (VNPT/Vĩnh Hy) bóc tách thuế VAT thù lao dịch vụ, tự động gửi link hóa đơn kèm lời cảm ơn qua Zalo OA cho khách hàng (UC011). Hồ sơ chuyển trạng thái **"Đã hoàn tất"** và tự động đồng bộ vào kho lưu trữ điện tử.

<a id="2.5.c-quy-trinh-nghiep-vu-chung-thuc-chu-ky"></a>
#### 2.5.C Quy trình Nghiệp vụ Chứng thực chữ ký (Signature Certification Workflow)

Quy trình sử dụng phần mềm đối với nghiệp vụ Chứng thực chữ ký (tờ khai, giấy ủy quyền, sơ yếu lý lịch...) gồm các bước sau:

1. **Tạo hồ sơ & Cấp số Nháp sớm (UC005):**
   - Thư ký tạo hồ sơ mới trên hệ thống, chọn dịch vụ **"Chứng thực chữ ký"**.
   - **Cấp số Nháp sớm:** Hệ thống tự động truy cập Sổ chứng thực chữ ký tương ứng để **xin cấp 1 số chứng thực sớm ở trạng thái Nháp (Draft / Tạm giữ)** để giữ chỗ trên Sổ điện tử, tránh bị trùng lặp số giữa các người dùng thao tác song song. Số Nháp này hiển thị trực tiếp trên giao diện hồ sơ.
...
3. **Nhập form thông tin khách hàng & Khởi tạo quản trị:**
   - Thư ký nhập thông tin định danh của người yêu cầu chứng thực vào form hồ sơ.
   - **Tra cứu CRM:** Thư ký nhập số CCCD hoặc SĐT của khách hàng, hệ thống tự động tra cứu CRM và điền (Auto-fill) các thông tin định danh tương ứng. Nếu là đương sự mới, sử dụng quét mã QR trên CCCD vật lý (hoặc phím tắt F8 dán từ Zalo) để điền nhanh.
   - Nếu văn bản yêu cầu nhiều người ký, Thư ký bấm **"Thêm người ký"** để nhập định danh của tất cả các chủ thể cùng tham gia.
   - **Nhập thông tin quản trị:** Thư ký chọn **Nguồn khách hàng** để tính KPI/hoa hồng; chọn **Người phụ trách** xử lý hồ sơ; và chọn **Công chứng viên (CCV)** chịu trách nhiệm ký duyệt hồ sơ.
4. **Báo phí & Thiết lập đặt cọc:**
   ...
   - Thư ký báo phí trọn gói với khách hàng (bao gồm cả phí dịch vụ in ấn, bìa hồ sơ nếu có). Nhập số tiền thỏa thuận thực thu vào trường `Tổng thực thu` (áp dụng cơ chế Price Override).
   - **Cơ chế Tạm ứng / Đặt cọc (nếu có):** Nhập số tiền tạm ứng / đặt cọc đóng trước hoặc nhập Mã tạm ứng / đặt cọc, nhấn **"Xác nhận đã nhận tạm ứng / đặt cọc"** để ghi nhận giao dịch cọc lên phần mềm.
5. **Checklist nghiệp vụ (động theo loại văn bản - UC006):**
...
6. **Sinh Lời chứng & In ấn (UC007):**
   - Thư ký nhấn nút "Sinh Lời chứng". Hệ thống tự động lấy thông tin từ form và **Số chứng thực Nháp đã cấp ở bước 1** điền vào biểu mẫu Lời chứng tương ứng (Ví dụ: Số: 1234, Quyển số: 01-CT).
   - Thư ký thực hiện in Lời chứng, dán kẹp vào văn bản.
7. **Ký offline:**
   - Khách hàng ký tên hoặc điểm chỉ trực tiếp lên văn bản giấy trước sự chứng kiến của Thư ký và CCV.
   - Công chứng viên (CCV) ký tên xác nhận lên Lời chứng vật lý và đóng dấu đỏ vật lý lên văn bản kẹp Lời chứng.
8. **CCV Đối soát số Nháp & Phê duyệt ký số CA (UC012):**
   - CCV đăng nhập hệ thống, mở hồ sơ thuộc danh sách "Chờ phê duyệt".
   - CCV đối soát chéo số chứng thực hiển thị trên màn hình (đang ở trạng thái **Nháp - Draft**) với số thứ tự thực tế ghi trên bản cứng giấy dấu đỏ.
   - CCV nhấn nút **"Phê duyệt & Ký số CA"**.
   - Hệ thống ghi nhận chữ ký số của CCV lên Lời chứng điện tử và chuyển trạng thái hồ sơ sang **"Chờ thu phí"** (số chứng thực vẫn giữ trạng thái Nháp).
   - *Trường hợp hủy hồ sơ:* CCV hoặc Thư ký bấm nút "Hủy hồ sơ" trên giao diện chi tiết, nhập lý do hủy. Hệ thống cập nhật trạng thái hồ sơ thành "Đã hủy (Có cấp số)" và chuyển trạng thái các số chứng thực tương ứng từ Nháp sang **Đã hủy (Cancelled)**.
9. **Thu phí tại quầy (UC009):**
   - Thư ký/Thu ngân tiến hành thu tiền mặt/chuyển khoản từ khách hàng theo tổng thực thu đã chốt.
   - Thư ký có thể điều chỉnh số trang/bản để recalculate thù lao và thuế VAT khớp với tiền thực tế thu.
   - Thư ký nhấn **"Xác nhận nhận đủ tiền"**. **Hệ thống tự động chuyển đổi trạng thái của số chứng thực tương ứng từ Nháp (Draft) sang Hoàn thành (Completed) trên Sổ điện tử**, chuyển trạng thái hồ sơ sang **"Chờ tải file scan"**.
   - *Trường hợp hủy hồ sơ tại quầy:* Chọn hủy hồ sơ, nhập lý do hủy. Hệ thống cập nhật số chứng thực tương ứng thành **Đã hủy (Cancelled)**.
10. **Hoàn tất form & Scan số hóa tài liệu (UC008):**
    - Thư ký nhập hoàn thiện các trường còn thiếu trên form hồ sơ (nếu trước đó đã bỏ qua ở bước 3).
    - Thư ký scan văn bản đã ký đóng dấu đầy đủ có chữ ký và dấu đỏ của CCV, tải file scan lên hệ thống (quét QR để tự động gán) và bấm **"Hoàn tất xử lý"**. Trạng thái hồ sơ chuyển sang "Chờ đối soát".
11. **Đối soát & Phát hành Hóa đơn (UC010):**
    ...
    - Hệ thống tự động gửi tin nhắn Zalo OA/SMS cảm ơn kèm liên kết tải hóa đơn điện tử cho khách hàng (UC011).
<a id="2.x-cau-hinh-bieu-phi-dich-thuat-linh-hoat"></a>
#### 2.5.D Quy tắc Nghiệp vụ: Cấu hình Biểu phí Dịch thuật Linh hoạt (Hybrid Pricing Model)
...
1. **Cấu hình bảng giá nền (Base Price Template):**
    ...
    ![Cấu hình Bảng giá & Dịch vụ](images/cau_hinh_bang_gia.png)
...
###### Giao diện mẫu (Prototype) - Đăng nhập (UC001):
![Đăng nhập hệ thống](images/dang_nhap.png)
...
###### Giao diện mẫu (Prototype) - Quản lý nhân sự & Phân quyền (UC003 & UC004):
![Quản lý nhân sự & Phân quyền](images/quan_ly_nguoidung.png)
...
| **Luồng sự kiện chính (Thành công)** | **STT \| Thực hiện bởi \| Hành động** <br>1. Thư ký \| Nhập Số điện thoại/CCCD (Cá nhân) hoặc Mã số thuế (Doanh nghiệp) vào ô tìm kiếm. <br>2. Hệ thống \| Tìm kiếm trong CRM: Nếu có dữ liệu cũ, tự động điền (Auto-fill) thông tin định danh. Nếu chưa có, mở form thêm mới khách hàng. <br>3. Thư ký \| Chọn loại nghiệp vụ (Sao y bản chính, Chứng thực chữ ký, Dịch thuật công chứng). <br>4. Thư ký \| Chọn loại giấy tờ (CCCD, Sổ đỏ, Bằng cấp, Giấy phép kinh doanh,...) từ danh sách dropdown. <br>5. Thư ký \| Nhập các thông số kỹ thuật (Số trang gốc, Số bản cần làm), chọn **Nguồn khách hàng** để tính KPI, chọn **Người phụ trách** xử lý, và chọn **Công chứng viên (CCV)** chịu trách nhiệm ký duyệt. <br>6. Hệ thống \| Tự động tính phí gốc Nhà nước theo công thức quy định, hiển thị bảng phân rã dịch vụ con và hiển thị chi tiết tại khối **"Tạm tính & Báo phí khách hàng"** (bao gồm: Tổng phí gốc Nhà nước, Tổng thù lao dịch vụ, Tổng cộng tạm tính). <br>7. Thư ký \| Nhập số tiền thực tế thống nhất thu vào trường "Tổng thực thu thực tế (Price Override)" (mặc định bằng Tổng tạm tính) và báo phí cho khách hàng. <br>8. Thư ký \| (Tùy chọn) Nhập số tiền tạm ứng / đặt cọc khách đóng vào ô input tương ứng tại khối Tạm tính, tích chọn checkbox `Khách hàng đồng ý báo phí` (bắt buộc) và checkbox `Xác nhận đã thu tiền tạm ứng này` (nếu khách đóng trước), sau đó nhấn nút **"Khởi tạo hồ sơ & Cấp số Nháp"**. <br>9. Hệ thống \| Khởi tạo hồ sơ, tự động sinh Mã hồ sơ duy nhất, cấp dải số công chứng sớm tự động dưới dạng **Nháp (Draft)** giữ chỗ trên Sổ điện tử, đặt trạng thái hồ sơ thành "Đang xử lý". |
...
| 7 | Tổng thực thu | Tiền thu thỏa thuận (Price Override) | Có | Số tiền $\ge$ Phí gốc nhà nước | 100000 |
| 8 | Người dịch | ID của Cộng tác viên dịch thuật | Chỉ khi là Dịch thuật | Phải tồn tại trong CSDL cộng tác viên | CTV-002 |
| 9 | Nguồn khách hàng | Nguồn gốc giới thiệu/khai thác hồ sơ | Có | Thuộc danh mục cấu hình nguồn để tính KPI | Nguồn cá nhân (Nguyễn Văn B) |
| 10 | Người phụ trách | Nhân sự nghiệp vụ phụ trách chính | Có | Phải là nhân sự có tài khoản hoạt động | secretary@notary.vn |
| 11 | Công chứng viên | CCV ký phê duyệt hồ sơ | Có | Phải là Công chứng viên hoạt động | ccv1@notary.vn |
| 12 | Số tiền tạm ứng / đặt cọc | Số tiền tạm ứng / đặt cọc đóng trước (nếu có) | Không | Số tiền $\ge$ 0 | 500000 |
| 13 | Mã tạm ứng / đặt cọc | Mã tạm ứng / đặt cọc tự quy định của văn phòng | Không | Đúng định dạng mã cấu hình | COC-500K-01 |
| 14 | Đã nhận tạm ứng / đặt cọc | Cờ xác nhận đã thu tạm ứng / đặt cọc | Không | Nhận giá trị Boolean | True |
| 15 | Đồng ý báo phí | Khách hàng đồng ý với mức phí báo | Có | Nhận giá trị Boolean (phải chọn True) | True |

###### Giao diện mẫu (Prototype) - Bàn làm việc & Khởi tạo hồ sơ:
![Bàn làm việc Hồ sơ nghiệp vụ](images/ho_so_nghiep_vu_hub.png)

![Danh sách quản lý hồ sơ](images/danh_sach_ho_so.png)

![Giao diện Khởi tạo hồ sơ](images/khoi_tao_ho_so.png)

###### Hộp thoại nổi (Modal) - Thêm dịch vụ nghiệp vụ con:
![Giao diện Thêm dịch vụ con (Modal)](images/khoi_tao_ho_so_modal.png)
...
###### Giao diện mẫu (Prototype) - Thẩm định & Checklist chặn (UC006):
![Thẩm định & Checklist chặn](images/tham_dinh_checklist.png)
...
![Soạn thảo Lời chứng tự động](images/soan_thao_loi_chung.png)

![Quản lý Biểu mẫu Lời chứng](images/quan_ly_bieu_mau.png)
...
| **Luồng sự kiện chính (Thành công)** | **STT \| Thực hiện bởi \| Hành động** <br>1. CCV \| Kiểm tra hồ sơ vật lý và đối soát thông tin trên phần mềm. <br>2. CCV \| Chọn loại Sổ tương ứng (Sổ Sao y, Sổ Hợp đồng, Sổ Chứng thực chữ ký, Sổ Bản dịch) trên giao diện. <br>3. Hệ thống \| Tự động hiển thị và gợi ý số thứ tự tiếp theo có sẵn trong Sổ đã chọn (STT = Số cuối cùng đã cấp trong năm + 1). <br>4. CCV \| (Tùy chọn) Chọn "Cấp số tự động" hoặc tự nhập số thủ công (Giữ số) nếu có độ trễ đồng bộ. <br>5. CCV \| Nhấn nút "Xác nhận Phê duyệt & Cấp số". <br>6. Hệ thống \| Phân tách hồ sơ thành các dòng dịch vụ con độc lập (nếu hồ sơ gộp nhiều dịch vụ), cấp số công chứng riêng biệt cho từng dịch vụ và ghi nhận vào các Sổ điện tử tương ứng. <br>7. Hệ thống \| Ghi nhật ký phê duyệt (CCV ký tên, timestamp) và chuyển trạng thái hồ sơ sang "Chờ thu phí" (hoặc gửi thông báo qua Zalo OA báo tạm ứng/cọc/thanh toán). |
...
![CCV Ký duyệt & Đóng dấu](images/ccv_phe_duyet.png)
...
| **Hậu điều kiện** | Hồ sơ số hóa được lưu trữ thành công; hồ sơ được chuyển sang hàng chờ đối soát của Kế toán. |
...
###### Giao diện mẫu (Prototype) - Quét & Lưu trữ hồ sơ (UC008):
![Quét & Lưu trữ hồ sơ](images/quet_luu_tru_ho_so.png)
...
![Giao diện Thu phí tại quầy](images/thu_phi_tai_quay.png)
...
###### Giao diện mẫu (Prototype) - Đối soát & Hóa đơn điện tử (UC010):
![Đối soát & Hóa đơn điện tử](images/doi_soat_hoa_don.png)
...
###### Giao diện mẫu (Prototype) - Chăm sóc khách hàng qua Zalo OA (UC011):
![Chăm sóc khách hàng qua Zalo OA](images/cham_soc_khach_hang_zalo.png)
...
###### Giao diện mẫu (Prototype) - Sổ công chứng điện tử (Đồng bộ & Cấp số theo dịch vụ):
![Sổ công chứng điện tử](images/so_cong_chung_dien_tu.png)
...
![Tổng quan Báo cáo Dashboard](images/tong_quan_dashboard.png)
...
| **Số công chứng** | **Mỗi dịch vụ con trong một hồ sơ (Đơn hàng) nhận một số công chứng riêng biệt.** Nếu đơn hàng có nhiều dịch vụ, mỗi dịch vụ con sẽ có mã số công chứng gắn kèm riêng lẻ để lưu trữ và quản lý sổ sách độc lập. |
...
### 3.1 Nghiệp vụ Sao y bản chính (Minh họa theo luồng HS-2026-0918)

Dưới đây là kịch bản minh họa luồng xử lý toàn diện một hồ sơ thực tế (Sao y bản chính kết hợp Dịch thuật) bám sát các màn hình giao diện đã thiết kế của hệ thống:

<a id="3.1.1-case-1-sao-y-cccd"></a>
#### 3.1.1 Bước 1: Tiếp nhận & Khởi tạo hồ sơ, Đặt cọc
- **Bối cảnh:** Khách hàng Nguyễn Văn A (SĐT: 0912345678) yêu cầu sao y và dịch thuật tài liệu sở hữu tài sản.
- **Thao tác trên hệ thống:**
  - Thư ký nhập SĐT, hệ thống tự động điền (Auto-fill) thông tin Khách hàng.
  - Tại phần **Thông tin dịch vụ & Quản trị**, Thư ký chọn "Nguồn khách hàng" (Tính KPI), phân công "Người phụ trách xử lý" (Thư ký Nguyễn Thùy Linh) và "Công chứng viên phê duyệt" (Lê Văn Đức).
  - Tại phần **Cấu hình Đặt cọc**, Thư ký nhập số tiền 500.000 VNĐ, nhập mã cọc `COC-500K-01`, tích chọn *Xác nhận đã nhận cọc từ khách hàng*.
  - Nhấn nút **"Lưu & Tiếp tục"** để chuyển sang bước Thẩm định.
  
  ![Khởi tạo hồ sơ](images/khoi_tao_ho_so.png)

<a id="3.1.2-case-2-sao-y-so-do"></a>
#### 3.1.2 Bước 2: Thẩm định & Checklist (Kiểm tra ngăn chặn UCHI)
- **Thao tác trên hệ thống:**
  - Màn hình chuyển sang giao diện Thẩm định hồ sơ `HS-2026-0918`.
  - Thư ký thực hiện rà soát theo **Bộ Checklist động tương ứng (Sao y bản chính & Dịch thuật)**: kiểm tra tính toàn vẹn văn bản, thông tin định danh, chữ ký CTV dịch thuật.
  - **Kiểm tra ngăn chặn tài sản qua UCHI API**: Hệ thống tự động kết nối CSDL Sở Tư pháp. Trạng thái an toàn trả về: *"Tài sản không nằm trong danh mục ngăn chặn/phong tỏa giao dịch."*
  - Thư ký nhấn nút **"Xác nhận & Tiến hành lời chứng"**. 
  - *(Nhánh rẽ: Nếu tài sản bị chặn hoặc sai phạm pháp lý, Thư ký nhấn nút **"Từ chối hồ sơ (Lỗi pháp lý)"**).*
  
  ![Thẩm định Checklist](images/tham_dinh_checklist.png)

<a id="3.1.3-case-3-sao-y-so-luong-lon"></a>
#### 3.1.3 Bước 3: Biên tập Lời chứng tự động
- **Thao tác trên hệ thống:**
  - Hệ thống chuyển sang giao diện Biên tập lời chứng. Nội dung thông tin khách hàng, số trang, số bản sao được tự động chèn vào biểu mẫu.
  - Thư ký rà soát lại văn bản hiển thị trên Editor, sau đó in lời chứng vật lý để kẹp vào hồ sơ trình CCV.
  
  ![Soạn thảo lời chứng](images/soan_thao_loi_chung.png)

<a id="3.1.4-case-4-doanh-nghiep-sao-y-giay-phep"></a>
#### 3.1.4 Bước 4: Quét (Scan) & Lưu trữ số hóa bản gốc
- **Thao tác trên hệ thống:**
  - Sau khi CCV đã ký và đóng dấu đỏ vật lý, Thư ký tiến hành scan tài liệu hoàn thiện.
  - Thư ký tải file PDF lên hệ thống qua tính năng Upload trên màn hình Quét & Lưu trữ.
  - Hệ thống tự động bóc tách chữ ký số và gắn metadata vào file lưu trữ.
  
  ![Quét lưu trữ hồ sơ](images/quet_luu_tru_ho_so.png)

#### 3.1.5 Các kịch bản rẽ nhánh khác (Edge Cases)

**A. Sao y số lượng lớn & Điều chỉnh giá linh hoạt**
- Nếu khách hàng sao y bộ hồ sơ thầu 1250 trang (tổng phí hàng triệu đồng), Kế toán có thể điều chỉnh số lượng trang trực tiếp trên hệ thống để khớp tuyệt đối với số tiền thỏa thuận thực thu mà không làm sai lệch luồng hóa đơn.

**B. Doanh nghiệp sao y giấy phép & Yêu cầu hóa đơn VAT (B2B)**
- Thư ký chỉ cần nhập Mã số thuế, hệ thống gọi API Tổng cục Thuế để tự động điền Tên công ty và Địa chỉ.
- Khi Kế toán hoàn tất đối soát tài chính, hệ thống tự động xuất hóa đơn điện tử B2B (qua VNPT/Vĩnh Hy) và gửi email trực tiếp cho doanh nghiệp.

**C. Sao y giấy tờ có yếu tố nước ngoài (Hợp pháp hóa lãnh sự)**
- Hệ thống kích hoạt checklist bắt buộc: `Đã kiểm tra tem và con dấu Hợp pháp hóa lãnh sự của Bộ Ngoại giao`.
- Nếu tài liệu chưa có tem, Thư ký nhấn **"Từ chối hồ sơ"** và in *Phiếu hướng dẫn hoàn thiện hồ sơ* cho khách hàng.
...
- **Luồng xử lý trên hệ thống:**
    ...
    - Thư ký báo phí trọn gói dịch vụ (bao gồm cả phí in ấn, bìa) là $200.000đ$. Nhập vào ô "Tổng thực thu". Hệ thống hạch toán phần chênh lệch $55.000đ$ vào mục "Thù lao dịch vụ khác". Thư ký tích chọn `Khách hàng đồng ý báo phí` và nhấn nút **"Khởi tạo hồ sơ & Cấp số Nháp"** để khởi tạo hồ sơ (`DT-YYYYMMDD-XXXXX`).
...
- **Luồng xử lý trên hệ thống:**
    ...
    - Do giá trị hồ sơ lớn, hệ thống tự động bật cờ cảnh báo `Yêu cầu tạm ứng`. Hệ thống khóa tính năng giao việc cho đến khi Thư ký thu trước tối thiểu 50% tiền tạm ứng / đặt cọc ($5.625.000đ$). Công ty X quẹt thẻ POS tạm ứng. Thư ký nhấn "Xác nhận đã nhận tạm ứng / đặt cọc", hệ thống mở khóa luồng xử lý tiếp theo.
...
Dưới đây là các kịch bản nghiệp vụ thực tế của phân hệ **Chứng thực chữ ký** nhằm cụ thể hóa quy trình 7 bước đã nêu tại mục 2.5.3, minh họa cách hệ thống xử lý từng loại văn bản khác nhau với checklist động, tích hợp tra cứu UCHI khi liên quan đến tài sản, và cơ chế auto-fill CRM:

<a id="3.3.1-case-1-chung-thuc-ban-tu-khai-ly-lich"></a>
#### 3.3.1 Case 1: Chứng thực chữ ký trên Bản tự khai lý lịch - Luồng siêu tốc
- **Bối cảnh:** Anh A cần chứng thực chữ ký trên 3 bản Tự khai lý lịch (có sẵn nội dung, đã tự điền thông tin cá nhân) để nộp hồ sơ xin việc tại công ty nước ngoài.
- **Hành vi thực tế:** Văn bản đơn giản, không liên quan đến tài sản, không cần tra cứu UCHI. Khách hàng muốn xử lý nhanh trong 5-10 phút.
- **Luồng xử lý trên hệ thống:**
    - **Bước 1 - Tạo hồ sơ:** Thư ký tạo hồ sơ mới, chọn dịch vụ **"Chứng thực chữ ký"**. Hệ thống cấp mã `CTCK-YYYYMMDD-XXXXX`.
...
- **Luồng xử lý trên hệ thống:**
    ...
    - **Bước 2 - Chọn loại văn bản:** Thư ký chọn loại **"Giấy ủy quyền nhận lương / bảo hiểm"** từ danh mục.
...
- **Luồng xử lý trên hệ thống:**
    ...
    - **Bước 5 - Checklist:** Hệ thống hiển thị checklist cho "Giấy ủy quyền xe máy / phương tiện":
      ...
      - Thư ký tích chọn hoàn thành 100%. Hệ thống mở khóa bước tiếp theo.
    - **Bước 6 - In ấn & Ký duyệt:** Thư ký in Lời chứng chứng thực chữ ký cá nhân (mẫu `TEMP-CTCK-CANHAN-01`), dán kẹp vào Giấy ủy quyền. Anh E ký tên trực tiếp lên Giấy ủy quyền trước sự chứng kiến của Thư ký và CCV. CCV ký Lời chứng và đóng dấu đỏ vật lý.
    - **Bước 7 - Hoàn tất & Thu phí:** Thư ký xin thông tin địa chỉ chi tiết của anh F để nhập bổ sung vào form (trước đó đã bỏ qua). Scan bản cứng đã ký đóng dấu tải lên hệ thống, bấm "Hoàn tất xử lý". Thu tiền $50.000đ$, nhấn "Xác nhận nhận đủ tiền". Kế toán đối soát, hệ thống xuất hóa đơn điện tử gửi Zalo OA cảm ơn cho anh E.