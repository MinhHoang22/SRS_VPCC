...
Tài liệu Đặc tả Yêu cầu Phần mềm (SRS) này xác định và đặc tả chi tiết toàn bộ các yêu cầu chức năng và bối cảnh hoạt động của **Hệ thống ERP/CRM Văn phòng Công chứng (NotaryOS)** được triển khai độc lập tại một văn phòng công chứng. Tài liệu đóng vai trò là:
...
### 1.2 Phạm vi (Scope)
NotaryOS là giải pháp quản trị tổng thể cho Văn phòng công chứng, giúp số hóa toàn bộ hoạt động vận hành thủ công lên một nền tảng chung. Hệ thống bao quát 3 lớp vận hành chính:
1. **Lớp Khách hàng (CRM):** Quản lý định danh cá nhân và doanh nghiệp, lịch sử giao dịch và tự động tương tác chăm sóc khách hàng qua Zalo OA.
...
Tài liệu này đặc tả toàn diện và chi tiết tất cả các yêu cầu chức năng cho toàn bộ hệ thống NotaryOS cũng như quy trình chi tiết của 3 nghiệp vụ cốt lõi được định nghĩa trong Chương 2:
...
Dự án NotaryOS được phát triển nhằm mục tiêu xây dựng một hệ thống quản trị tổng thể (ERP/CRM) chuyên biệt cho văn phòng công chứng. Hệ thống số hóa toàn bộ quy trình tiếp nhận hồ sơ tại quầy, thẩm định pháp lý và kiểm soát lỗi qua checklist chặn, tra cứu dữ liệu ngăn chặn giao dịch (UCHI), quản lý dòng tiền hạch toán và tự động xuất hóa đơn VAT, cũng như chăm sóc khách hàng tự động qua Zalo OA. NotaryOS giúp tối ưu hóa hiệu suất làm việc liên phòng ban (Thư ký, Công chứng viên, Kế toán, Admin), giảm thiểu 95% sai sót nghiệp vụ và nâng cao trải nghiệm dịch vụ khách hàng.
...
Hệ thống NotaryOS định nghĩa 4 tác nhân (vai trò người dùng) trực tiếp tương tác và vận hành trên hệ thống:
...
1. **Thư ký nghiệp vụ (Secretary):**
   ...
   - **Nhiệm vụ trên hệ thống:**
     ...
     - Khởi tạo hồ sơ cho toàn bộ nghiệp vụ cốt lõi, nhập các thông số đầu vào (loại giấy tờ, số trang, số bản).
...
4. **Quản trị viên (Admin):**
   - **Mô tả:** Nhân sự quản trị kỹ thuật, cấu hình hệ thống NotaryOS.
   - **Nhiệm vụ trên hệ thống:**
...
Các chức năng cốt lõi của hệ thống NotaryOS được phân chia theo 3 lớp vận hành chính:
...
- **Auto-fill & Tra cứu nhanh:** Tự động điền thông tin khách hàng cũ dựa vào SĐT hoặc MST (liên kết API Tổng cục Thuế) khi tạo hồ sơ mới.
...
- **Quản lý tính phí nghiệp vụ:** Tự động tính phí gốc nhà nước theo trang/bản, bóc tách thù lao dịch vụ khác, và tự động phân tách hóa đơn con khi phí gốc vượt trần pháp luật quy định ($\le 200.000đ$). Ngoài ra, cho phép nhập phí thủ công đối với trường hợp cấu hình bảng phí chưa có (ví dụ chưa có tính phí cho dịch từ ngôn ngữ A sang B)
...
- **Phân quyền RBAC:** Cấu hình ma trận quyền chi tiết (Xem, Thêm, Sửa, Xóa mềm, Duyệt) cho từng vai trò đối với từng chức năng.
- **Nhật ký hệ thống (Audit Trail):** Ghi nhận bất biến lịch sử thao tác của toàn bộ người dùng trên hệ thống.
...
#### 2.4.1 Phân rã nhóm chức năng Xác thực & Quản trị (Auth & Admin)
Nhóm chức năng này phục vụ quản lý tài khoản nhân viên, thiết lập mật khẩu và phân quyền hoạt động trong hệ thống NotaryOS:
...
flowchart LR
    ...
    subgraph Auth_Admin_Decomposed ["Phân rã Xác thực & Quản trị"]
...
Nhóm chức năng nghiệp vụ xử lý tài liệu do Thư ký phụ trách, có sự tham gia phê duyệt nội dung trực tuyến của Công chứng viên:
...
flowchart LR
    ...
    subgraph Dossier_Decomposed ["Phân rã Nghiệp vụ Hồ sơ"]
        ...
        UC007("UC007: Trộn dữ liệu & Soạn thảo Lời chứng")
...
Nhóm chức năng hạch toán dòng tiền và thanh toán do Kế toán phụ trách, kết hợp với các tiến trình tự động hóa của hệ thống:

```mermaid
flowchart LR
    Acc(["💰 Kế toán"])

    subgraph Billing_Decomposed ["Phân rã Tài chính & Hóa đơn"]
        UC009("UC009: Thu phí & Đối soát dòng tiền")
        UC010("UC010: Xuất hóa đơn điện tử tự động")
        UC011("UC011: Chăm sóc khách hàng tự động")
...
Quy trình vận hành chung trên hệ thống NotaryOS được điều phối qua 5 bước chính:
...
3. **Ký duyệt (Thư ký & Công chứng viên):** Sau khi hoàn thành checklist và kết quả tra cứu hợp lệ, Thư ký click in lời chứng/hợp đồng đã trộn dữ liệu tự động. Thư ký kẹp bản in trình CCV duyệt. CCV xem danh sách chờ trên màn hình NotaryOS của mình và ký đóng dấu đỏ vật lý lên bản cứng offline. Thư ký nhận lại bản cứng, scan chụp ảnh và tải lên hệ thống để lưu trữ điện tử, mở chặn luồng kế toán.
...
| **Mô tả** | Người dùng đăng nhập vào hệ thống NotaryOS bằng tài khoản (Email) và mật khẩu để bắt đầu phiên làm việc. |
...
| **Sự kiện kích hoạt** | Admin truy cập màn hình "Cấu hình phân quyền" trên hệ thống NotaryOS. |
| **Tiền điều kiện** | Admin đã đăng nhập thành công vào hệ thống. |
